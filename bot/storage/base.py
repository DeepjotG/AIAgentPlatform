...
in-memory implementation for Postgres touches one line in ``main.py``.
"""
...
class Repository(ABC):
    async def setup(self) -> None:
        """Open connections and create tables. No-op for in-memory."""
    ...
    async def list_pending_channel_ids(self) -> list[int]:
        """Every gated channel. Read once at startup by ``IntakeService.hydrate``."""
